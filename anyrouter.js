...
// Built at: 2025-12-02T10:24:38.873Z
...
var init_config = __esm({
  "src/config.js"() {
    BUILD_TIME = "2025-12-02T10:24:38.873Z";
    FALLBACK_CONFIG = {};
...
async function recordRequest(env, data) {
  ...
  try {
    ...
    if (keyId) {
      ...
      commands.push(["HSET", `${STATS_PREFIX}:lastused`, keyId, (/* @__PURE__ */ new Date()).toISOString()]);
    }
    if (ip && ip !== "unknown") {
...
<body class="gradient-bg min-h-screen">
      ...
      <div class="glass-effect rounded-2xl shadow-xl mini-card mb-6 card-hover">
        ...
        <div class="grid grid-cols-1 md:grid-cols-12 gap-3">
          ...
          <div class="md:col-span-3">
            ...
            <input type="text" id="newRemark" placeholder="\u53EF\u9009\u5907\u6CE8\u8BF4\u660E" maxlength="255" class="w-full mini-input border border-gray-200 rounded-lg focus:outline-none focus:border-purple-500 transition-all">
          </div>
  ...
  <script>
    ...
    $('#addBtn').click(async function() {
      ...
      btn.prop('disabled', true).html('<i class="fas fa-spinner fa-spin"></i>');
      try {
        const response = await fetch('/api/configs', { method: 'POST', headers: { 'Content-Type': 'application/json', 'Authorization': 'Bearer ' + authToken }, body: JSON.stringify({ api_url: apiUrl, token, enabled, remark }) });
        const result = await response.json();
        if (result.success) { $('#newApiUrl').val(''); $('#newToken').val(''); $('#newRemark').val(''); loadConfigs(); showToast('\u6DFB\u52A0\u6210\u529F', 'success'); } else { showToast('\u6DFB\u52A0\u5931\u8D25: ' + result.error, 'error'); }
    ...
    function renderConfigs(configs) {
      ...
      Object.entries(configs).forEach(([apiUrl, config]) => {
        config.keys.forEach(key => { rows.push({ id: key.id, key_id: key.key_id, sk_alias: key.sk_alias || null, api_url: apiUrl, token: key.token, enabled: key.enabled, remark: key.remark || '', created_at: key.created_at, updated_at: key.updated_at }); });
      });
      currentConfigs = rows;
    ...
    window.openEditModal = function(id) { const config = currentConfigs.find(c => c.id === id); if (!config) return; currentEditId = id; $('#editApiUrl').val(config.api_url); $('#editToken').val(config.token); $('#editRemark').val(config.remark || ''); $('#editEnabled').val(config.enabled.toString()); $('#editModal').removeClass('hidden'); };
    ...
    $('#saveEditBtn').click(async function() {
      ...
      btn.prop('disabled', true).html('<i class="fas fa-spinner fa-spin"></i>');
      try {
        const response = await fetch('/api/configs/' + currentEditId, { method: 'PATCH', headers: { 'Content-Type': 'application/json', 'Authorization': 'Bearer ' + authToken }, body: JSON.stringify({ token, enabled, remark }) });
        const result = await response.json();
